...
problem = MKP("Benchmarks_Chu", pNo)
# problem = OneMax(2000)
runtime = 30
K = 2
...
for run in range(runtime):
    ...
    for operator_selector in operator_selectors:
        ...
        abc = BinaryABC(problem, operator_pool, operator_selector, pop_size=20, maxFE=100,
                        limit=100)
        for operator in operator_pool:
            operator.set_algorithm(abc)
        operator_selector.set_algorithm(abc)
        abc.run()
        convergence_logs = Log(abc.convergence, 'results', f'cg-{problem.benchmark_name}-{problem.file_no}', abc.operator_selector.__conf__())
